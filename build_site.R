...
<div class="container">
  ...
  <p>
    ...
    legislator &times; vote matrix \\(\\mathbf{X}\\), SVD factorises it as \\(\\mathbf{X} = \\mathbf{U}\\mathbf{D}\\mathbf{V}^\\top\\), where \\(\\mathbf{U}\\) gives each
...
<div class="container">
  ...
  <div class="callout">
    \\[X_{ij}^{\\mathrm{imp}} = \\bar{X}_{i\\cdot} + \\bar{X}_{\\cdot j} - \\bar{X}_{\\cdot\\cdot} \\quad \\text{(only where } X_{ij} \\text{ is missing)}\\]
  </div>
...
<span class="kw">for</span> (k <span class="kw">in</span> <span class="fn">seq_len</span>(<span class="fn">nrow</span>(na_idx))) {
  ...
  <p>
    One apparent anomaly deserves attention: Die Linke, a far-left party ideologically,
    appears alongside CDU/CSU and AfD in vote-space &mdash; not with the Ampel coalition
    parties. This is not a coding error. Die Linke was in opposition throughout the 20th
    Bundestag and consistently voted against Ampel legislation, just as CDU/CSU and AfD did,
    though for entirely different ideological reasons. The first SVD dimension therefore
    conflates <em>ideological position</em> with <em>coalition membership</em>. The
    double-centered SVD (Section 02) partially addresses this by removing additive voting
    tendencies.
  </p>
  <p>
    BSW (Bündnis Sahra Wagenknecht) entered the Bundestag in January 2024 after splitting
    from Die Linke and has fewer roll-call votes than other parties, resulting in higher
  ...
  <p>
    Note: some positive-loading votes may appear cross-partisan or surprising. This reflects
    the government-vs-opposition structure of dimension 1 &mdash; any motion that the
    opposition (CDU/CSU + AfD) voted for en bloc receives a positive loading, regardless of
  ...
  <div class="reveal">
  <p class="section-bridge" style="color:var(--text-muted);font-style:italic;border-top:1px solid var(--border);padding-top:.9rem;margin-top:1.4rem">
    Having established that a single dimension dominates Bundestag voting, we now ask whether
    this structure persists after removing additive row and column effects &mdash; legislators&rsquo;
    overall yes-vote tendencies and bills&rsquo; overall passage rates.
  </p>
  </div>
</div>
</section>
...
<span class="fn">max</span>(<span class="fn">abs</span>(<span class="fn">colMeans</span>(X_dc)))  <span class="cm"># @@MAX_COL_ERR@@</span></pre>
  ...
  <div class="reveal">
  <p class="section-bridge" style="color:var(--text-muted);font-style:italic;border-top:1px solid var(--border);padding-top:.9rem;margin-top:1.4rem">
    Both SVD approaches recover the same underlying structure, but neither provides
    uncertainty estimates for individual legislators. In Section 03 we turn to a fully
    Bayesian model that addresses exactly this limitation.
  </p>
  </div>
</div>
</section>
...
prior_2pl <span class="kw">&lt;-</span>
  ...
  <p>
    ...
    \\(r = @@COR_SVD_IRT@@\\). This strong agreement validates both methods: SVD
  ...
  <div class="figure-block reveal cap-visible">
    ...
    <div class="figure-caption">
      <strong>Figure 7.</strong> The strong linear relationship between SVD dim-1 scores and
      IRT posterior means across all @@N_LEGISLATORS@@ legislators (\\(r = @@COR_SVD_IRT@@\\)).
      Dashed line = OLS fit; dotted line = y&nbsp;=&nbsp;x reference. Hover for name and party.
    </div>
  </div>
  <div class="reveal">
  <p class="section-bridge" style="color:var(--text-muted);font-style:italic;border-top:1px solid var(--border);padding-top:.9rem;margin-top:1.4rem">
    With ideal points and their posterior distributions in hand, we can now make explicit
    probability statements about ideological orderings &mdash; the central advantage of the
    Bayesian approach over SVD.
  </p>
  </div>
</div>
</section>
...
p_afd_gt_cdu <span class="kw">&lt;-</span> <span class="fn">mean</span>(afd_draws <span class="kw">&gt;</span> cdu_draws)  <span class="cm"># = @@P_AFD_RAW@@</span></pre>
  ...
  <div class="callout">
    ...
    roughly \\(1/\\sqrt{n}\\). The posterior probability that AfD&rsquo;s mean exceeds CDU/CSU&rsquo;s
  ...
  <p class="section-bridge" style="color:var(--text-muted);font-style:italic;border-top:1px solid var(--border);padding-top:.9rem;margin-top:1.4rem">
    Section 05 provides an interactive explorer where you can examine any legislator&rsquo;s
    position directly. Section 06 investigates how sensitive these conclusions are to the
    choice of prior.
  </p>
  </div>
</div>
...
<div class="container">
  <div class="section-header reveal">
    ...
    <span class="key-finding">Three methods, one signal: government-vs-opposition dominates 20th Bundestag voting</span>
  </div>
  <div class="reveal">
  <p>
    Three complementary methods &mdash; imputed SVD, double-centered SVD, and Bayesian 2PL
    IRT &mdash; converge on the same finding: the 20th German Bundestag is overwhelmingly
    one-dimensional in its roll-call voting behaviour. The dominant axis separates the Ampel
    governing coalition (SPD, Gr&uuml;nen, FDP) from the opposition (CDU/CSU, AfD), with AfD
    sitting furthest from the coalition pole. This structure is robust to the choice of
    estimation method (\\(r = @@COR_SVD_IRT@@\\) between SVD and IRT) and to the choice of
    prior (\\(r = @@COR_HS_BASE@@\\) between normal and horseshoe).
  </p>
  <p>
    The Bayesian IRT model adds what SVD cannot: calibrated uncertainty. Party orderings are
    near-certain under the posterior &mdash; P(&theta;&#772;_AfD &gt; &theta;&#772;_CDU) =
    <strong>@@P_AFD_GT_CDU@@</strong> across all 500 draws &mdash; even though individual
    legislators carry substantial uncertainty. The dominant dimension should not be na&iuml;vely
    labelled left&ndash;right ideology: it captures government-vs-opposition voting dynamics,
    which in a coalition system conflate ideological position with cabinet membership.
    Disentangling the two would require analysing only non-government-sponsored bills or
    legislatures with mid-term coalition changes.
  </p>
  </div>
</div>
</section>

<!-- ── SECTION 08: AI WORKFLOW ───────────────────────────────────────────── -->
<section id="prompts">
<div class="container">
  <div class="section-header reveal">
  ...
  <h3>Claude.ai Planning Sessions</h3>
  <p>
    In addition to Claude Code, the project used Claude.ai (browser chat) for assignment
    planning, prompt engineering, and iterative site critique. Key contributions:
  </p>
  <ul style="padding-left:1.4rem;margin-top:.5rem;line-height:2">
    <li>Clarified the brms 2PL specification (B&uuml;rkner 2021 formulation, 1 chain 500 samples
        as per course instructions)</li>
    <li>Designed the initial Claude Code prompt structure and subsequent improvement prompts
        across multiple passes</li>
    <li>Reviewed site output iteratively and identified substantive issues: Die Linke
        government&ndash;opposition anomaly, posterior sign contradictions, party ordering,
        missing bridging text and conclusion</li>
    <li>Discussed methodological questions including convergence warnings, sampling
        specifications, and horseshoe prior interpretation</li>
    <li>Generated all Claude Code prompts used in this project</li>
  </ul>
  <p style="margin-top:.8rem">
    <strong>Prompt engineering approach:</strong> Prompts were designed to be explicit and
    sequential, with each Claude Code session reading existing files before making changes
    and confirming each step before proceeding. Iterative critique cycles between Claude.ai
    and Claude Code were used to improve both analytical content and site design across
    approximately 8 prompt passes.
  </p>
...
/* Official Bundestag seating order (bottom-to-top for strip y-axis) */
const BUNDESTAG_ORDER = ["fraktionslos","AfD","CDU/CSU","FDP","BÜNDNIS 90/DIE GRÜNEN","SPD","BSW","Die Linke"];
function partyOrderByMean(field) {
  const present = new Set(RAW_DATA.filter(d=>d[field]!=null).map(d=>d.party));
  return BUNDESTAG_ORDER.filter(p => present.has(p));
}
...
function scatterTraces(xf,yf,labelFn,hl) {
  const present = new Set(RAW_DATA.map(d=>d.party));
  const parties = [...BUNDESTAG_ORDER].reverse().filter(p=>present.has(p));
...
lazyPlot("fig2-container", function() {
  Plotly.newPlot("fig2-container",
    ...
    Object.assign({},LAY_BASE,{
      ...
      yaxis:Object.assign({},AX,{automargin:true,categoryorder:"array",categoryarray:BUNDESTAG_ORDER.map(sp)}),
...
lazyPlot("fig8-container", function() {
  ...
  const order = BUNDESTAG_ORDER.filter(p => byParty[p]);
...
/* =========================================================================
   SECTION 05 — Interactive explorer
   ========================================================================= */
const _present5 = new Set(RAW_DATA.map(d=>d.party));
const allParties = [...BUNDESTAG_ORDER].reverse().filter(p=>_present5.has(p));
